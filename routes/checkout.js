...
router.route("/create-checkout-session").post(async (req, res) => {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: process.env.PRODUCTION_SUCCESS || "http://localhost:3000/success?id={CHECKOUT_SESSION_ID}",
      cancel_url: process.env.PRODUCTION_CANCEL || "http://localhost:3000/",
    });
